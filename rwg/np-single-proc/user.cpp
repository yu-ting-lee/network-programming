...
User::User(Client &cli) {
  envs = {Env("PATH", "bin:.")};
  addr = ((std::string)inet_ntoa(cli.addr.sin_addr) + ":" +
          std::to_string(ntohs(cli.addr.sin_port)));
  name = "(no name)";
  fd = cli.fd;
}
...
void User::set_upipe_input() {
  if (ufds[0] != nullptr) {
    utils::dup2(ufds[0][0], STDIN_FILENO);
    utils::close_pipe(ufds[0]);
...
void User::set_upipe_output() {
  if (ufds[1] != nullptr) {
    utils::dup2(ufds[1][1], STDOUT_FILENO);
    utils::close_pipe(ufds[1]);
...
void User::execvp(args_t &args) {
  ...
  if (nfds[1] == nullptr && ufds[1] == nullptr) {
